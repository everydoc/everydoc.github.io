...
这个教程的目的是教大家一起使用spider-flow这款流星的爬虫软件。
...
>spider-flow 是一个爬虫平台，以图形化方式定义爬虫流程，无需代码即可实现一个爬虫。接下来我们将依次介绍spider-flow爬虫平台的部署、基础使用、插件使用、爬虫开发等功能。为了方便大家的使用，我们已经为大家制作好docker镜像，大家可以直接拉取镜像使用。
>
>1. 初始化数据库，我们执行源代码中提供初始化脚本来创建并初始化一些数据。
>2. 启动spider-flow，我们从中央仓库拉取镜像文件，并根据说明更改脚本内容，然后执行启动脚本。
>3. 打开浏览器输入指定的端口地址即可
